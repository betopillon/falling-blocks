...
class ObstacleRect extends PositionComponent with HasGameReference<UghGame>, CollisionCallbacks {
  ...
  ObstacleRect({required this.speed});
  ...
  void render(Canvas canvas) {
    canvas.drawRect(size.toRect(), Paint()..color = obsColor);
  }
  ...
  void update(double dt) {
    ...
    if (position.y > gameRef.size.y + size.y) {
      ...
      final BonusHighlights highlight = BonusHighlights(
        label: (size.x.toInt() + (speed * 2.0).toInt()).toString(),
      )..position = Vector2(position.x, position.y - 100);
      gameRef.add(highlight);

      if (gameRef.player.isAlive) {
        gameRef.player.updateScore(size.x.toInt() + (speed * 2.0).toInt());
      }
    }
  }
